    ...
    for SNR in SNRset:  # 0 0.5 1 1.5 2 2.5 3
        ...
        for ik in range(0, max_batches):  # max_batches 3
            ...
            if top_config.update_llr_with_epdf:
                ...
                loss += np.sum(np.mean(np.square(s_mod_plus_res_noise-s_mod), 1))  # axis=1 表示计算[[a,b],[c,d]] [(a+b)/2, (c+d)/2]，损失函数是均方误差
        ...
        if top_config.update_llr_with_epdf:
            ...
            loss /= np.double(simutimes)  # 猜测 simutimes = 5000*3, 其中5000是一次测试5000行马元，3是测试三次即 max_batches = 3
